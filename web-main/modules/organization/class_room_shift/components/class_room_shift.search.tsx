...
interface SearchInputProps
  ...
  placeholder?: string;
  detail?: ClassRoomShift;
  class_room?: string
}

const ClassRoomShiftSearchInput: React.FC<SearchInputProps> = ({
  placeholder = "Search ClassRoomShifts",
  value,
  detail,
  onChange,
  ...restProps
  ...
  const fetchData = useCallback(
    async (term: string = "", page: number = 1, append: boolean = false) => {
      setLoading(true);
      try {
        const { data: { results, next } } = await classroomshiftService.getClassRoomShifts({
          search: term?.trim(),
          ps: 100,
          pn: page,
        });

        const formatted = results?.map((classroomshift: ClassRoomShift) => ({
          value: classroomshift.id,
          label: `${formatTo12Hour(classroomshift.start_time)} - ${formatTo12Hour(classroomshift.end_time)}`,
        }));
  ...
  useEffect(() => {
    if (detail && value) {
      setOptions(prev => {
        ...
        if (!exists) {
          return [...(prev || []), { value: detail.id, label: `${formatTo12Hour(detail.start_time)} - ${formatTo12Hour(detail.end_time)}`}];
        }
        return prev;
      });
    }
...
export default ClassRoomShiftSearchInput;
