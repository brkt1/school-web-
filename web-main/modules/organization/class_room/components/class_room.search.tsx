...
const ClassRoomSearchInput: React.FC<SearchInputProps> = ({
  ...
  const fetchData = useCallback(
    async (term: string = "", page: number = 1, append: boolean = false) => {
      setLoading(true);
      try {
        const { data: { results, next } } = await classroomService.getClassRooms({
          search: term?.trim(),
          ps: 100,
          pn: page,
          institution
        ...
        const formatted = results?.map((classroom: ClassRoom) => ({
          value: classroom.id,
...
export default ClassRoomSearchInput;
