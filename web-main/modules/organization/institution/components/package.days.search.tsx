...
const PackageDaysSearchInput: React.FC<SearchInputProps> = ({
  institution,
  package_id,
  ...
  const fetchData = useCallback(
    async (term: string = "", page: number = 1, append: boolean = false) => {
        if(!institution || !package_id){
          return;
        }
      setLoading(true);
      try {
        const { data: { results, next } } = await institutionService.getInstitutionPackageDays(institution, package_id,{
          search: term?.trim(),
          ps: 100,
          pn: page,
        });
    ...
    [institutionService, institution, package_id]
  );
...
export default PackageDaysSearchInput;
