...
import { Button, Dropdown, Input, Popconfirm, Table, Tag } from "antd";
...
interface InstitutionListProps extends Partial<Institution>, Navigations {

}

const InstitutionList: React.FC<InstitutionListProps> = (props) => {
...
const [selectedRowKeys, setSelectedRowKeys] = useState<React.Key[]>([]);
  ...
  const fetchData = useCallback(() => {
    setLoading(true);
    service.getInstitutions(requestParams)
      .then((res) => {
  ...
  const handleDelete = useCallback((id: string) => {
    service.deleteInstitution(id).then(() => fetchData());
  }, []);

  const getMenuItems = useCallback((item: Institution): MenuProps["items"] => [
    {
      label: <Link className="flex gap-0.5 items-baseline"  href={`${props.edit_navigation || "/admin/organization/institutions"}/${item.id}/edit`}><FaEdit />Edit</Link>,
      key: "edit",
    },
    {
      label: (
  ...
  const columns: ColumnsType<Institution> = useMemo(() => [

        {
        key: "name",
        title: "Name",
        dataIndex: "name",
        render: (_, row) => <Link href={`${props.list_navigation || "/admin/organization/institutions"}/${row.id}`}>{row.name}</Link>,
        sorter: { multiple: 1 },
        },
        ...
        title: "Level",
        dataIndex: "level",
        render: (_, row) => <ul>{row.level_detail.map(data => <Tag>{data.name}</Tag>)}</ul>,
        sorter: { multiple: 1 },
        },
...
const rowSelection: TableRowSelection<Institution> = {
...
selections: [Table.SELECTION_ALL, Table.SELECTION_INVERT, Table.SELECTION_NONE],
  ...
  const handleTableChange: TableProps<Institution>["onChange"] = (pagination, filters, sorter) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="w-full flex justify-between">
        ...
        <Button href={props.add_navigation || `/admin/organization/institutions/create`} type="primary" icon={<FaPlus />}>Add</Button>
      </div>
