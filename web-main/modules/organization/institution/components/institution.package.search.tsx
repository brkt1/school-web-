...
const InstitutionPackageSearchInput: React.FC<SearchInputProps> = ({
  institution,
  ...
  const fetchData = useCallback(
    async (term: string = "", page: number = 1, append: boolean = false) => {
      ...
      setLoading(true);
      try {
        const { data: { results, next } } = await institutionService.getInstitutionPackages(institution,{
          search: term?.trim(),
          ps: 100,
          pn: page,
        });

        const formatted = results?.map((feePackage: Package) => ({
          value: feePackage.id,
...
export default InstitutionPackageSearchInput;
