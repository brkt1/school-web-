...
const [selectedRowKeys, setSelectedRowKeys] = useState<React.Key[]>([]);
  ...
  const fetchData = useCallback(() => {
    setLoading(true);
    service.getRegions(requestParams)
      .then((res) => {
  ...
  const handleDelete = useCallback((id: string) => {
    service.deleteRegion(id).then(() => fetchData());
  }, []);

  const getMenuItems = useCallback((item: Region): MenuProps["items"] => [
    {
      label: <Link className="flex gap-0.5 items-baseline"  href={`/admin/organization/regions/${item.id}/edit`}><FaEdit />Edit</Link>,
      key: "edit",
    },
    {
      label: (
  ...
  const columns: ColumnsType<Region> = useMemo(() => [
        ...
        title: "Name",
        dataIndex: "name",
        render: (_, row) => <Link href={`/admin/organization/regions/${row.id}`}>{row.name}</Link>,
        sorter: { multiple: 1 },
        },
...
const rowSelection: TableRowSelection<Region> = {
...
selections: [Table.SELECTION_ALL, Table.SELECTION_INVERT, Table.SELECTION_NONE],
  ...
  const handleTableChange: TableProps<Region>["onChange"] = (pagination, filters, sorter) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="w-full flex justify-between">
        ...
        <Button href={`/admin/organization/regions/create`} type="primary" icon={<FaPlus />}>Add</Button>
      </div>
