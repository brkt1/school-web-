...
interface SearchInputProps
  ...
  placeholder?: string;
  detail?: StudentClassShift;
}

const StudentClassShiftSearchInput: React.FC<SearchInputProps> = ({
  placeholder = "Search StudentClassShifts",
  value,
  detail,
  onChange,
  ...restProps
  ...
  const fetchData = useCallback(
    async (term: string = "", page: number = 1, append: boolean = false) => {
      setLoading(true);
      try {
        const { data: { results, next } } = await studentclassshiftService.getStudentClassShifts({
          search: term?.trim(),
          ps: 10,
          pn: page,
        });

        const formatted = results?.map((studentclassshift: StudentClassShift) => ({
          value: studentclassshift.id,
              label: studentclassshift.class_room_detail.name,
        }));
...
export default StudentClassShiftSearchInput;
