...
interface SearchInputProps
  ...
  placeholder?: string;
  detail?: TeacherClassShift;
}

const TeacherClassShiftSearchInput: React.FC<SearchInputProps> = ({
  placeholder = "Search TeacherClassShifts",
  value,
  detail,
  onChange,
  ...restProps
  ...
  const fetchData = useCallback(
    async (term: string = "", page: number = 1, append: boolean = false) => {
      setLoading(true);
      try {
        const { data: { results, next } } = await teacherclassshiftService.getTeacherClassShifts({
          search: term?.trim(),
          ps: 100,
          pn: page,
        });

        const formatted = results?.map((teacherclassshift: TeacherClassShift) => ({
          value: teacherclassshift.id,
              label: teacherclassshift.class_room_detail.name,
        }));
  ...
  useEffect(() => {
    if (detail && value) {
      setOptions(prev => {
        ...
        if (!exists) {
          return [...(prev || []), { value: detail.id, label: detail.class_room_detail.name }];
        }
        return prev;
      });
    }
  }, [detail?.id, detail?.class_room_detail.name, value]);
...
export default TeacherClassShiftSearchInput;
