...
interface SearchInputProps
  ...
  placeholder?: string;
  detail?: StudentRequestShift;
}

const StudentRequestShiftSearchInput: React.FC<SearchInputProps> = ({
  placeholder = "Search StudentRequestShifts",
  value,
  detail,
  onChange,
  ...restProps
  ...
  const fetchData = useCallback(
    async (term: string = "", page: number = 1, append: boolean = false) => {
      setLoading(true);
      try {
        const { data: { results, next } } = await studentrequestshiftService.getStudentRequestShifts({
          search: term?.trim(),
          ps: 100,
          pn: page,
        });

        const formatted = results?.map((studentrequestshift: StudentRequestShift) => ({
          value: studentrequestshift.id,
              label: studentrequestshift.student_detail.user_detail.full_name,
        }));
  ...
  useEffect(() => {
    if (detail && value) {
      setOptions(prev => {
        ...
        if (!exists) {
          return [...(prev || []), { value: detail.id, label: detail.student_detail.user_detail.full_name }];
        }
        return prev;
      });
    }
  }, [detail?.id, detail?.student_detail.user_detail.full_name, value]);
...
export default StudentRequestShiftSearchInput;
