...
import FeePackageSearchInput from "@/modules/finance/fee_package/components/fee_package.search";
import { Days, TeacherType } from "../../student/student.enum";
import { TableRowSelection } from "antd/es/table/interface";

interface StudentRequestShiftListProps extends Partial<StudentRequestShift>, Navigations {
  active?: boolean
  has_action?: boolean;
}

const StudentRequestShiftList: React.FC<StudentRequestShiftListProps> = ({has_action=true, ...props}) => {
...
const [selectedRowKeys, setSelectedRowKeys] = useState<React.Key[]>([]);
  ...
  const fetchData = useCallback(() => {
    setLoading(true);
    service.getStudentRequestShifts(requestParams)
      .then((res) => {
  ...
  const handleDelete = useCallback((id: string) => {
    service.deleteStudentRequestShift(id).then(() => fetchData());
  }, []);

  const getMenuItems = useCallback((item: StudentRequestShift): MenuProps["items"] => [
    {
      label: <Link className="flex gap-0.5 items-baseline" href={`${props.list_navigation || "/admin/accounts/student_request_shifts"}/${item.id}/edit`}><FaEdit />Edit</Link>,
      key: "edit",
    },
    {
      label: (
  ...
  const columns: ColumnsType<StudentRequestShift> = useMemo(() => [
        {
        key: "student",
        title: "Student",
        dataIndex: "student",
        ...
        render: (_, row) => row.fee_package_detail.name,
        filterDropdown: ({setSelectedKeys, confirm }) => (
          <div style={{ padding: 8 }}>
            <FeePackageSearchInput
              defaultOpen
              onChange={(value) => {
                setSelectedKeys(value ? [value] : []);
                confirm();
...
const rowSelection: TableRowSelection<StudentRequestShift> = {
...
selections: [Table.SELECTION_ALL, Table.SELECTION_INVERT, Table.SELECTION_NONE],
  ...
  const handleTableChange: TableProps<StudentRequestShift>["onChange"] = (pagination, filters, sorter) => {
...
{ has_action &&  <Button href={props.add_navigation || `/admin/accounts/student_request_shifts/create`} type="primary" icon={<FaPlus />}>Add</Button> }      </div>
...
export default StudentRequestShiftList;
