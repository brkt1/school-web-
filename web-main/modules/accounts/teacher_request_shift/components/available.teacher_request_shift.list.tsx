...
import useTeacherService from "../../teacher/teacher.service";
...
import useTeacherClassShiftService from "../../teacher_class_shift/teacher_class_shift.service";
import { TeacherClassShift } from "../../teacher_class_shift/teacher_class_shift.model";
import PackageSearchInput from "@/modules/lookup/package/components/package.search";
import { TableRowSelection } from "antd/es/table/interface";

interface AvailableTeacherRequestShiftListProps extends Partial<ClassRoomShift>, Navigations {
  teacher: string
  active?: boolean
}

const AvailableTeacherRequestShiftList: React.FC<AvailableTeacherRequestShiftListProps> = (props) => {
...
const [selectedRowKeys, setSelectedRowKeys] = useState<React.Key[]>([]);
  ...
  const fetchData = useCallback(() => {
    setLoading(true);
    service.getTeacherAvailableShifts(props.teacher)
      .then((res) => {
  ...
  const assignClass = (class_room_shift: ClassRoomShift) => {
    setLoading(true)
    teacherclassshiftService.addTeacherClassShift({ ...class_room_shift, teacher: props.teacher, institution: class_room_shift.class_room_detail.institution, class_room_shift: class_room_shift.id } as unknown as TeacherClassShift)
    .then((value) => {
      fetchData()
    })
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="w-full flex justify-between">
        ...
        {/* <Button href={props.add_navigation || `/admin/accounts/teacher_request_shifts/create`} type="primary" icon={<FaPlus />}>Add</Button> */}
      </div>
...
export default AvailableTeacherRequestShiftList;
