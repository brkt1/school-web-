...
interface SearchInputProps
  ...
  placeholder?: string;
  detail?: TeacherRequestShift;
}

const TeacherRequestShiftSearchInput: React.FC<SearchInputProps> = ({
  placeholder = "Search TeacherRequestShifts",
  value,
  detail,
  onChange,
  ...restProps
  ...
  const fetchData = useCallback(
    async (term: string = "", page: number = 1, append: boolean = false) => {
      setLoading(true);
      try {
        const { data: { results, next } } = await teacherrequestshiftService.getTeacherRequestShifts({
          search: term?.trim(),
          ps: 100,
          pn: page,
        });

        const formatted = results?.map((teacherrequestshift: TeacherRequestShift) => ({
          value: teacherrequestshift.id,
              label: teacherrequestshift.teacher_detail.user_detail.full_name,
        }));
...
export default TeacherRequestShiftSearchInput;
