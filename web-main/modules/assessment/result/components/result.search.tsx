...
const ResultSearchInput: React.FC<SearchInputProps> = ({
  ...
  const fetchData = useCallback(
    async (term: string = "", page: number = 1, append: boolean = false) => {
      setLoading(true);
      try {
        const { data: { results, next } } = await resultService.getResults({
          search: term?.trim(),
          ps: 100,
          pn: page,
        });

        const formatted = results?.map((result: Result) => ({
          value: result.id,
              label: result.student_detail.user_detail.full_name,
        }));
