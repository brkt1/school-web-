...
const [selectedRowKeys, setSelectedRowKeys] = useState<React.Key[]>([]);
  ...
  const fetchData = useCallback(() => {
    setLoading(true);
    service.getResults(requestParams)
      .then((res) => {
  ...
  const handleDelete = useCallback((id: string) => {
    service.deleteResult(id).then(() => fetchData());
  }, []);

  const getMenuItems = useCallback((item: Result): MenuProps["items"] => [
    {
      label: <Link className="flex gap-0.5 items-baseline"  href={`/admin/assessment/results/${item.id}/edit`}><FaEdit />Edit</Link>,
      key: "edit",
    },
    {
      label: (
  ...
  const columns: ColumnsType<Result> = useMemo(() => [
        ...
        title: "Date",
        dataIndex: "date",
        render: (_, row) => toDateAndTime(row.date),
        sorter: { multiple: 1 },
        },
    {
      title: "Actions",
...
const rowSelection: TableRowSelection<Result> = {
...
selections: [Table.SELECTION_ALL, Table.SELECTION_INVERT, Table.SELECTION_NONE],
  ...
  const handleTableChange: TableProps<Result>["onChange"] = (pagination, filters, sorter) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="w-full flex justify-between">
        ...
        <Button href={`/admin/assessment/results/create`} type="primary" icon={<FaPlus />}>Add</Button>
      </div>
