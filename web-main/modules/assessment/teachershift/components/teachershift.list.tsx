...
const TeacherShiftList: React.FC = () => {
...
const [selectedRowKeys, setSelectedRowKeys] = useState<React.Key[]>([]);
  ...
  const fetchData = useCallback(() => {
    setLoading(true);
    service.getTeacherShifts(requestParams)
      .then((res) => {
  ...
  const handleDelete = useCallback((id: string) => {
    service.deleteTeacherShift(id).then(() => fetchData());
  }, []);

  const getMenuItems = useCallback((item: TeacherShift): MenuProps["items"] => [
    {
      label: <Link className="flex gap-0.5 items-baseline"  href={`/admin/assessment/teachershifts/${item.id}/edit`}><FaEdit />Edit</Link>,
      key: "edit",
    },
    {
      label: (
  ...
  const columns: ColumnsType<TeacherShift> = useMemo(() => [
        ...
        title: "Day",
        dataIndex: "day",
        render: (_, row) => getEnumName(Days, row.day),
        sorter: { multiple: 1 },
...
const rowSelection: TableRowSelection<TeacherShift> = {
...
selections: [Table.SELECTION_ALL, Table.SELECTION_INVERT, Table.SELECTION_NONE],
  ...
  const handleTableChange: TableProps<TeacherShift>["onChange"] = (pagination, filters, sorter) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="w-full flex justify-between">
        ...
        <Button href={`/admin/assessment/teachershifts/create`} type="primary" icon={<FaPlus />}>Add</Button>
      </div>
...
export default TeacherShiftList;
