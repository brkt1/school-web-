...
const [selectedRowKeys, setSelectedRowKeys] = useState<React.Key[]>([]);
  ...
  const fetchData = useCallback(() => {
    setLoading(true);
    service.getAttendances(requestParams)
      .then((res) => {
  ...
  const handleDelete = useCallback((id: string) => {
    service.deleteAttendance(id).then(() => fetchData());
  }, []);

  const getMenuItems = useCallback((item: Attendance): MenuProps["items"] => [
    {
      label: <Link className="flex gap-0.5 items-baseline"  href={`/admin/assessment/attendances/${item.id}/edit`}><FaEdit />Edit</Link>,
      key: "edit",
    },
    {
      label: (
...
selections: [Table.SELECTION_ALL, Table.SELECTION_INVERT, Table.SELECTION_NONE],
  ...
  const handleTableChange: TableProps<Attendance>["onChange"] = (pagination, filters, sorter) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="w-full flex justify-between">
        ...
        <Button href={`/admin/assessment/attendances/create`} type="primary" icon={<FaPlus />}>Add</Button>
      </div>
