...
const AttendanceSearchInput: React.FC<SearchInputProps> = ({
  ...
  const fetchData = useCallback(
    async (term: string = "", page: number = 1, append: boolean = false) => {
      setLoading(true);
      try {
        const { data: { results, next } } = await attendanceService.getAttendances({
          search: term?.trim(),
          ps: 100,
          pn: page,
        });

        const formatted = results?.map((attendance: Attendance) => ({
          value: attendance.id,
              label: attendance.student_detail.user_detail.full_name,
        }));
