...
const ApplicationSearchInput: React.FC<SearchInputProps> = ({
  ...
  const fetchData = useCallback(
    async (term: string = "", page: number = 1, append: boolean = false) => {
      setLoading(true);
      try {
        const { data: { results, next } } = await applicationService.getApplications({
          search: term?.trim(),
          ps: 100,
          pn: page,
        });

        const formatted = results?.map((application: Application) => ({
          value: application.id,
              label: application.teacher_detail.user_detail.full_name,
        }));
