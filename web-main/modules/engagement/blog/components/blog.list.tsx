...
interface BlogListProps extends Partial<Blog>, Navigations {
...
const [selectedRowKeys, setSelectedRowKeys] = useState<React.Key[]>([]);
  ...
  const fetchData = useCallback(() => {
    setLoading(true);
    service.getBlogs(requestParams)
      .then((res) => {
  ...
  const handleDelete = useCallback((id: string) => {
    service.deleteBlog(id).then(() => fetchData());
  }, []);

  const getMenuItems = useCallback((item: Blog): MenuProps["items"] => [
    {
      label: <Link className="flex gap-0.5 items-baseline" href={`${props.list_navigation || "/admin/engagement/blogs"}/${item.id}/edit`}><FaEdit />Edit</Link>,
      key: "edit",
    },
    {
      label: (
  ...
  const columns: ColumnsType<Blog> = useMemo(() => [
        ...
        title: "Title",
        dataIndex: "title",
        render: (_, row) => <Link href={`/admin/engagement/blogs/${row.id}`}>{row.title}</Link>,
        sorter: { multiple: 1 },
        },
        ...
        title: "Thumbnail",
        dataIndex: "thumbnail",
        render: (_, row) => <Image height={50} src={row.thumbnail} />,
        sorter: { multiple: 1 },
        },
        {
        key: "create_date",
...
const rowSelection: TableRowSelection<Blog> = {
...
selections: [Table.SELECTION_ALL, Table.SELECTION_INVERT, Table.SELECTION_NONE],
  ...
  const handleTableChange: TableProps<Blog>["onChange"] = (pagination, filters, sorter) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="w-full flex justify-between">
        ...
        <Button href={props.add_navigation || `/admin/engagement/blogs/create`} type="primary" icon={<FaPlus />}>Add</Button>
      </div>
