...
interface TestimonialListProps extends Partial<Testimonial>, Navigations {

}

const TestimonialList: React.FC<TestimonialListProps> = (props) => {
...
const [selectedRowKeys, setSelectedRowKeys] = useState<React.Key[]>([]);
  ...
  const fetchData = useCallback(() => {
    setLoading(true);
    service.getTestimonials(requestParams)
      .then((res) => {
  ...
  const handleDelete = useCallback((id: string) => {
    service.deleteTestimonial(id).then(() => fetchData());
  }, []);

  const getMenuItems = useCallback((item: Testimonial): MenuProps["items"] => [
    {
      label: <Link className="flex gap-0.5 items-baseline" href={`${props.list_navigation || "/admin/engagement/testimonials"}/${item.id}/edit`}><FaEdit />Edit</Link>,
      key: "edit",
    },
    {
      label: (
  ...
  const columns: ColumnsType<Testimonial> = useMemo(() => [
        {
        key: "name",
        title: "Name",
        dataIndex: "name",
...
const rowSelection: TableRowSelection<Testimonial> = {
...
selections: [Table.SELECTION_ALL, Table.SELECTION_INVERT, Table.SELECTION_NONE],
  ...
  const handleTableChange: TableProps<Testimonial>["onChange"] = (pagination, filters, sorter) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="w-full flex justify-between">
        ...
        <Button href={props.add_navigation || `/admin/engagement/testimonials/create`} type="primary" icon={<FaPlus />}>Add</Button>
      </div>
