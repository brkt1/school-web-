...
interface ContactUsListProps extends Partial<ContactUs>, Navigations {

}

const ContactUsList: React.FC<ContactUsListProps> = (props) => {
...
const [selectedRowKeys, setSelectedRowKeys] = useState<React.Key[]>([]);
  ...
  const fetchData = useCallback(() => {
    setLoading(true);
    service.getContactUss(requestParams)
      .then((res) => {
  ...
  const handleDelete = useCallback((id: string) => {
    service.deleteContactUs(id).then(() => fetchData());
  }, []);

  const getMenuItems = useCallback((item: ContactUs): MenuProps["items"] => [
    {
      label: <Link className="flex gap-0.5 items-baseline" href={`${props.list_navigation || "/admin/feedback/contact_uss"}/${item.id}/edit`}><FaEdit />Edit</Link>,
      key: "edit",
    },
    {
      label: (
  ...
  const columns: ColumnsType<ContactUs> = useMemo(() => [
        {
        key: "full_name",
        title: "Full Name",
        dataIndex: "full_name",
...
selections: [Table.SELECTION_ALL, Table.SELECTION_INVERT, Table.SELECTION_NONE],
  ...
  const handleTableChange: TableProps<ContactUs>["onChange"] = (pagination, filters, sorter) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="w-full flex justify-between">
        ...
        <Button href={props.add_navigation || `/admin/feedback/contact_uss/create`} type="primary" icon={<FaPlus />}>Add</Button>
      </div>
