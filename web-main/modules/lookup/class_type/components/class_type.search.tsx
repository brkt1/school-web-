...
const ClassTypeSearchInput: React.FC<SearchInputProps> = ({
  placeholder = "Search ClassTypes",
  value,
  detail,
  onChange,
  ...restProps
  ...
  const fetchData = useCallback(
    async (term: string = "", page: number = 1, append: boolean = false) => {
      setLoading(true);
      try {
        const { data: { results, next } } = await classtypeService.getClassTypes({
          search: term?.trim(),
          ps: 100,
          pn: page,
        });

        const formatted = results?.map((classtype: ClassType) => ({
          value: classtype.id,
...
export default ClassTypeSearchInput;
