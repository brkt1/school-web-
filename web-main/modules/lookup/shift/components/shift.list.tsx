...
const [selectedRowKeys, setSelectedRowKeys] = useState<React.Key[]>([]);
  ...
  const fetchData = useCallback(() => {
    setLoading(true);
    service.getShifts(requestParams)
      .then((res) => {
  ...
  const handleDelete = useCallback((id: string) => {
    service.deleteShift(id).then(() => fetchData());
  }, []);

  const getMenuItems = useCallback((item: Shift): MenuProps["items"] => [
    {
      label: <Link className="flex gap-0.5 items-baseline"  href={`/admin/lookup/shifts/${item.id}/edit`}><FaEdit />Edit</Link>,
      key: "edit",
    },
    {
      label: (
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="w-full flex justify-between">
        ...
        <Button href={`/admin/lookup/shifts/create`} type="primary" icon={<FaPlus />}>Add</Button>
      </div>
