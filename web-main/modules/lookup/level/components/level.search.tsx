...
const LevelSearchInput: React.FC<SearchInputProps> = ({
  ...
  const fetchData = useCallback(
    async (term: string = "", page: number = 1, append: boolean = false) => {
      setLoading(true);
      try {
        const { data: { results, next } } = await levelService.getLevels({
          search: term?.trim(),
          ps: 100,
          pn: page,
        });

        const formatted = results?.map((level: Level) => ({
          value: level.id,
              label: level.name,
        }));
  ...
  useEffect(() => {
    if (detail && value) {
      ...
      setOptions(prev => {
        const exists = prev?.some(opt => detail.some(d => d.id === opt.value));
        if (!exists) {
          return [...(prev || []), ...detail.map(data => ({value: data.id, label: data.name}))];
        }
        return prev;
      });
    } else {
...
export default LevelSearchInput;
