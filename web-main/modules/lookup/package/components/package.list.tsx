...
const [selectedRowKeys, setSelectedRowKeys] = useState<React.Key[]>([]);
  ...
  const fetchData = useCallback(() => {
    setLoading(true);
    service.getPackages(requestParams)
      .then((res) => {
  ...
  const handleDelete = useCallback((id: string) => {
    service.deletePackage(id).then(() => fetchData());
  }, []);

  const getMenuItems = useCallback((item: Package): MenuProps["items"] => [
    {
      label: <Link className="flex gap-0.5 items-baseline"  href={`/admin/lookup/packages/${item.id}/edit`}><FaEdit />Edit</Link>,
      key: "edit",
    },
    {
      label: (
...
const rowSelection: TableRowSelection<Package> = {
...
selections: [Table.SELECTION_ALL, Table.SELECTION_INVERT, Table.SELECTION_NONE],
  ...
  const handleTableChange: TableProps<Package>["onChange"] = (pagination, filters, sorter) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="w-full flex justify-between">
        ...
        <Button href={`/admin/lookup/packages/create`} type="primary" icon={<FaPlus />}>Add</Button>
      </div>
