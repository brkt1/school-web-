...
const PaymentSearchInput: React.FC<SearchInputProps> = ({
  ...
  const fetchData = useCallback(
    async (term: string = "", page: number = 1, append: boolean = false) => {
      setLoading(true);
      try {
        const { data: { results, next } } = await paymentService.getPayments({
          search: term?.trim(),
          ps: 100,
          pn: page,
        });

        const formatted = results?.results?.map((payment: Payment) => ({
          value: payment.id,
              label: payment.ref_number,
        }));
  ...
  useEffect(() => {
    if (detail && value) {
      setOptions(prev => {
        ...
        if (!exists) {
          return [...(prev || []), { value: detail.id, label: detail.ref_number }];
        }
        return prev;
      });
    }
  }, [detail?.id, detail?.ref_number, value]);
